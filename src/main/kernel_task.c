...
///------------------------------------------------------------------------------------------------
///------------------------- commadn Queue for receving data  from the interrupts/callbacks to the kernel task

#define KERNEL_TASK_CMD_Q_LEN  10
#define KERNEL_TASK_CMD_Q_SIZE sizeof(uint32_t)

/// @brief wait for 50 millisecond to send the data to kernel cmd q
#define KERNEL_CMD_Q_SEND_WAIT pdMS_TO_TICKS(50)

/* The variable used to hold the queue's data structure. */
static StaticQueue_t Kernel_task_q_buffer;
/// q storage area
uint8_t Kernel_task_cmd_q_storage[KERNEL_TASK_CMD_Q_LEN * KERNEL_TASK_CMD_Q_SIZE];

static QueueHandle_t kernel_cmd_q_handle;
...
#define kernel_send_cmd(x)                                                                                                                 \
    uint32_t cmd = x;                                                                                                                      \
    xQueueSend(kernel_cmd_q_handle, &cmd, KERNEL_CMD_Q_SEND_WAIT);
...
void Kernel_task_preinit(void)
{
    ...
    /// create a static q cmd q for the kernel task
    kernel_cmd_q_handle =
        xQueueCreateStatic(KERNEL_TASK_CMD_Q_LEN, KERNEL_TASK_CMD_Q_SIZE, Kernel_task_cmd_q_storage, &Kernel_task_q_buffer);

        /// check the queue handle
        if (kernel_cmd_q_handle == NULL) {
            NRF_LOG_ERROR("kernel_q_failed");
    }

    /// create a kernel task
    kernnel_Task_handle = xTaskCreateStatic(kernel_task,
                                            KERNEL_TASK_NAME,
                                            NRF_KERNEL_TASK_STACK_DEPTH,
                                            NULL,
                                            KERNEL_TASK_PRIORITY,
                                            kernel_task_stack_buffer,
                                            &kernel_Task_TCB);
...
{
    /// delet the task and queue
    vTaskDelete(kernnel_Task_handle);
    /// delete the Queue
    vQueueDelete(kernel_cmd_q_handle);
}
...
static void ble_device_connected_callback(void* param, ble_gap_evt_t const* gap_evt)
{
    ...
    uint32_t ret = ble_gap_set_conn_handle(BLE_GAP_DEVICE_INDEX, gap_evt->conn_handle);
    NRF_ASSERT(ret);
    kernel_send_cmd(kernel_state_ble_connected);
}
...
static void ble_device_disconnected_callback(void* param, ble_gap_evt_t const* gap_evt)
{
    ...
    kernel_send_cmd(kernel_state_ble_disconnected);
}
...
static void ble_client_handle_value_notification(ble_gattc_evt_t const* gattc_evt)
{
    ...
    kernel_Task_woken = ble_ancs_client_event_handler(gattc_evt);
        ...
        if (kernel_Task_woken) { //// switch the kernel task state
            kernel_send_cmd(kernel_state_ble_notif_recv);
    }
}
...
static void ble_client_handle_value_indication(ble_gattc_evt_t const* gattc_evt)
{
    ...
    kernel_send_cmd(kernel_state_ble_indic_recv);
}
